...
public class WordLetter {
	...
	/**
	 * Método que permite generar las cartas (formato Word (docx))a ser enviadas
	 * a los usuarios con los datos personales que les permitan emitir su voto
	 * electrónico
	 * 
	 * @param voter,
	...
	public void generateLetter(Voter voter) throws Docx4JException {
		...
		letter.getMainDocumentPart().addStyledParagraphOfText("Title",
				"Datos de censo electoral para voto electrónico");
		letter.getMainDocumentPart().addStyledParagraphOfText("Subtitle",
				"Estimado " + voter.getNombre() + " con NIF: " + voter
						.getNif()
		...
		letter.getMainDocumentPart().addParagraphOfText("COLEGIO ELECTORAL: " + voter.getColegio().getId())
				.setPPr(paragraphFormat());
		letter.getMainDocumentPart().addStyledParagraphOfText("Subtitle",
