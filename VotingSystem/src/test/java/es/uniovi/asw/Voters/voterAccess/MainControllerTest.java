...
public class MainControllerTest {
	...
	/*
	 * @Test public void getUser() throws Exception { String userURI =
	 * base.toString() + "/user";
	 * 
	 * UserInfo expected = new UserInfo("David", "1234546789J", "uo212486",
	 * "123A"); UserPass requestData = new UserPass("uo212486", "password");
	 * 
	 * // retorna error 406. Puede que falte el accept aplication/json, no se.
	 * ResponseEntity<UserInfo> response = template.postForEntity(userURI,
	 * requestData, UserInfo.class);
	 * 
	...
	public void printScreen() throws Exception {
		MvcResult m = (mvc.perform(post("/user").contentType(MediaType.APPLICATION_JSON)
				...
				.content("{\"login\":\"user1@mail.com\", \"password\": \"user1\"}")).andReturn());
	...
	// //Voter voter1 = new Voter("User1", "user1@mail.com", "user1",
	// "12312321Q", 123);
	//
	// mvc.perform(post("/user")
	// .contentType(MediaType.APPLICATION_JSON)
	...
	public void postUserBadPass() throws Exception {
		mvc.perform(post("/user").contentType(MediaType.APPLICATION_JSON)
				.content("{\"login\":\"user1@mail.com\", \"password\": \"pas\"}")).andExpect(status().isNotFound());
	}

	@Test
	public void postUserInvalidMailFormat_at() throws Exception {
		mvc.perform(post("/user").contentType(MediaType.APPLICATION_JSON)
				.content("{\"login\":\"user1mail.com\", \"password\": \"user1\"}")).andExpect(status().isNotFound());
	}

	@Test
	public void postUserInvalidMailFormat_domain() throws Exception {
		mvc.perform(post("/user").contentType(MediaType.APPLICATION_JSON)
				.content("{\"login\":\"user1@gmail.com\", \"password\": \"user1\"}")).andExpect(status().isNotFound());
	}

	@Test
	public void postUserInvalidMailFormatDomain_wrongPass() throws Exception {
		mvc.perform(post("/user").contentType(MediaType.APPLICATION_JSON)
				.content("{\"login\":\"user1@mail.com\", \"password\": \"pa\"}")).andExpect(status().isNotFound());
	}
	...
	// .contentType(MediaType.APPLICATION_JSON)
	// .content("{\"login\":\"user1@mail.com\", \"oldPassword\": \"user1\",
	// \"newPassword\": \"newPass\"}")
	// ).andExpect(status().isOk());
	//
	...
	public void postUserChangePassErrorChangePass() throws Exception {
		...
		mvc.perform(post("/ChangePassword").contentType(MediaType.APPLICATION_JSON).content("null"))
				.andExpect(status().is4xxClientError());
	}
